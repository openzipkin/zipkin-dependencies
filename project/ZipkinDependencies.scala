...
object ZipkinDependencies extends Build {
  import Settings._
...
object Dependencies {
  ...
  object Compile {
    ...
    val zipkin            = "io.zipkin"           % "zipkin-scrooge"                      % Zipkin
    val zipkinCassandra   = "io.zipkin"           % "zipkin-cassandra"                    % Zipkin
    val thrift            = "org.apache.thrift"   % "libthrift"                           % Thrift force()
  }
  ...
  val app = connector ++ // json ++ scalaz ++ test ++
    Seq(thrift, zipkin, zipkinCassandra, "org.slf4j" % "slf4j-log4j12" % "1.6.1")

  val examples = connector ++ time ++ json ++
    Seq(kafka, kafkaStreaming, sparkML, zipkin, zipkinCassandra, "org.slf4j" % "slf4j-log4j12" % "1.6.1")
}
