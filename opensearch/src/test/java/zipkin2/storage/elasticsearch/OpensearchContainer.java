...
class OpensearchContainer extends GenericContainer<OpensearchContainer> {
  static final Logger LOGGER = LoggerFactory.getLogger(OpensearchContainer.class);

  OpensearchContainer(int majorVersion) {
    super(parse("ghcr.io/openzipkin/zipkin-opensearch2" + majorVersion + ":3.4.0"));
    addExposedPort(9200);
  ...
  ElasticsearchStorage.Builder newStorageBuilder() {
    ...
    builder.decorator((delegate, ctx, req) -> {
      ...
      return HttpResponse.of(response.aggregate().thenApply(r -> {
        // OS will return a 'warning' response header when using deprecated api, detect this and
  ...
  void processDependencies(ElasticsearchStorage storage, List<Span> spans) throws IOException {
    ...
    for (long day : days) {
      OpensearchDependenciesJob.builder().hosts(hostPort()).day(day).build().run();
    }
  }
}
